...
PORTFOLIO_COLUMNS_ = [
    'report_month', 'bucketed_balance', 'is_vip_or_prv', 'drop_flg',
   'optional_flg', 'bucketed_period', 'open_month', 'close_month',
   'bucketed_open_rate', 'report_dt', 'total_interests', 'remaining_interests',
    'weight_rate', 'total_generation', 'total_generation_cleared', 'weight_renewal_cnt', 'weight_renewal_available_flg', 'weight_close_plan_day',
    'gen_name', 'SER_d',
    'SER_d_cl', 'SER_dinamic', 'SER_dinamic_cl', 'total_generation_lag1', 'total_generation_cl_lag1', 'CUR', 
    ...
    'report_wo_period_weight_open_rate', 'init_total_generation', 'row_count','share_period_plan',
...
def get_spreads():
    ...
    spreads = pd.read_excel('/home/vtb70186744/dynbalance/examples/net_perc_income/spreads_deposits.xlsx')
    ...
    for i, date in enumerate(dates_list):
        for sub in [0,1]:
            if not ((sub, date) in spreads.index):
                #print((sub, date))
...
def get_ftp():
    ...
    ftp_table = pd.read_csv('/home/vtb70186744/dynbalance/examples/net_perc_income/ftp.csv')
    ...
    new_ftp = ((ftp_table[(ftp_table['report_year']==2022)&(ftp_table['report_month']==2)][cols]*w1).values \
               + (ftp_table[(ftp_table['report_year']==2022)&(ftp_table['report_month']==3)][cols]*w2).values)/(w1+w2)
...
def margin_calc(df):
    ...
    FOR = get_for()
    portfolio_res['report_month'] = portfolio_res['report_dt'].apply(lambda x: str(x)[:7])
