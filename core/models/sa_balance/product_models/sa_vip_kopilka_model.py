...
CONFIG = {
    ...
    "model_name": gen_sa_product_balance_model_name(product="kopilka", segment="vip"),
    "prediction_type": "kopilka_avg_balance",
}


class SaKopilkaVipModel:
...
class SaKopilkaVipDataLoader(SimpleDataLoader):
    def __init__(self, config=CONFIG):
...
class SaKopilkaVipModelTrainer(SimpleModelTrainer):
    def __init__(self, config=CONFIG) -> None:
        for key, value in config.items():
            setattr(self, key, value)
        self.dataloader = SaKopilkaVipDataLoader()
        self.model = SaKopilkaVipModel()


class SaKopilkaVipModelAdapter(SimpleModelAdapter):
    def __init__(
...
SaKopilkaVip = ModelMetaInfo(
    model_name=CONFIG["model_name"],
    model_trainer=SaKopilkaVipModelTrainer(),
    data_loader=SaKopilkaVipDataLoader(),
    adapter=SaKopilkaVipModelAdapter,
    segment="vip",
)
