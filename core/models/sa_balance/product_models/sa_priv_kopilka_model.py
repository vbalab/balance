...
CONFIG = {
    ...
    "full_features": None,
    "target": [
        "SA_avg_balance_[kopilka]_[priv]",
    ],
    ...
    "default_start_date": datetime(2019, 2, 1),
    "model_name": gen_sa_product_balance_model_name(product="kopilka", segment="priv"),
    "prediction_type": "kopilka_avg_balance",
}


class SaKopilkaPrivModel:
...
class SaKopilkaPrivDataLoader(SimpleDataLoader):
    def __init__(self, config=CONFIG):
...
class SaKopilkaPrivModelTrainer(SimpleModelTrainer):
    def __init__(self, config=CONFIG) -> None:
        for key, value in config.items():
            setattr(self, key, value)
        self.dataloader = SaKopilkaPrivDataLoader()
        self.model = SaKopilkaPrivModel()


class SaKopilkaPrivModelAdapter(SimpleModelAdapter):
    def __init__(
...
SaKopilkaPriv = ModelMetaInfo(
    model_name=CONFIG["model_name"],
    model_trainer=SaKopilkaPrivModelTrainer(),
    data_loader=SaKopilkaPrivDataLoader(),
    adapter=SaKopilkaPrivModelAdapter,
    segment="priv",
)
