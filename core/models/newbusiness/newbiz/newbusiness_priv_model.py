from sklearn.svm import SVR
from sklearn.pipeline import Pipeline
from sklearn.preprocessing import StandardScaler
from core.models.utils import (
    generate_svo_flg,
    verify_data,
    gen_newbiz_model_name,
    calculate_weighted_ftp_rate,
)
import pandas as pd
import numpy as np
from core.models.newbusiness.simple_adapters import (
    SimpleDataLoader,
    SimpleModelTrainer,
    SimpleModelAdapter,
)
from core.upfm.commons import (
    _REPORT_DT_COLUMN,
    ModelInfo,
    ForecastContext,
    ModelMetaInfo,
)
from datetime import datetime
from typing import Dict, Any

CONFIG = {
    "features": [
        #'VTB_weighted_rate_[priv]_[r1s1]',
        "VTB_weighted_rate_[priv]_[r0s0]",
        "VTB_weighted_ftp_rate_[priv]",
        "VTB_weighted_rate_[priv]",
        "rate_sa_weighted",
        "plan_close_[priv]",
        "SBER_max_rate",
    ],
    "full_features": [
        "spread_[VTB_weighted_rate_VTB_weighted_rate[r0s0]]",
        "spread_[VTB_weighted_ftp_rate_VTB_weighted_rate[r0s0]]",
        "december_flg",
        "spread_[VTB_weighted_rate_rate_sa_weighted]",
        "svo_flg",
        "prod_[plan_close_VTB_weighted_ftp_rate]",
        "delta_VTB_weighted_ftp_rate",
    ],
    "target": ["y_inflow_[priv]"],
    "estimator": Pipeline(
        [
            ("scaler", StandardScaler()),
            ("regressor", SVR(C=5, epsilon=5, kernel="linear")),
        ]
    ),
    "min_model_predict": 10e9,
    "table_name": "prod_dadm_alm_sbx.almde_fl_dpst_newbusiness_model_features",
    "table_date_col": _REPORT_DT_COLUMN,
    "default_start_date": datetime(2014, 1, 1),
    "model_name": gen_newbiz_model_name(segment="priv"),
}


class NewbusinessPrivModel:
    def __init__(self, config=CONFIG):
        for key, value in config.items():
            setattr(self, key, value)

    SP_NAME = "spread_[VTB_weighted_rate_SBER_max_rate]"

    def _sigmoid(self, x):
        return 1 / (1 + np.exp(-x))

    def _generate_features(self, X):
        # X = deepcopy(X)
        X_exog = pd.DataFrame(index=X.index)

        X_exog.loc[:, "december_flg"] = (X.index.month == 12).astype(float)

        X_exog.loc[:, "spread_[VTB_weighted_rate_rate_sa_weighted]"] = (
            X.loc[:, "VTB_weighted_rate_[priv]"] - X.loc[:, "rate_sa_weighted"]
        )
        X_exog.loc[
            X.index < "2020-05-01", "spread_[VTB_weighted_rate_rate_sa_weighted]"
        ] = 0.0

        # Добавляем функцию активации при спреде больше нуля
        X_exog.loc[
            X_exog["spread_[VTB_weighted_rate_rate_sa_weighted]"] > 0,
            "spread_[VTB_weighted_rate_rate_sa_weighted]",
        ] = X_exog.loc[
            X_exog["spread_[VTB_weighted_rate_rate_sa_weighted]"] > 0,
            "spread_[VTB_weighted_rate_rate_sa_weighted]",
        ].apply(
            lambda x: self._sigmoid(x)
        )

        X_exog.loc[:, "prod_[plan_close_VTB_weighted_ftp_rate]"] = (
            abs(X.loc[:, "plan_close_[priv]"])
            * X.loc[:, "VTB_weighted_ftp_rate_[priv]"]
        )
        X_exog.loc[:, "delta_VTB_weighted_ftp_rate"] = (
            X.loc[:, "VTB_weighted_ftp_rate_[priv]"].diff(1).fillna(0)
        )

        X_exog.loc[:, "spread_[VTB_weighted_ftp_rate_VTB_weighted_rate[r0s0]]"] = (
            X.loc[:, "VTB_weighted_ftp_rate_[priv]"]
            - X.loc[:, "VTB_weighted_rate_[priv]_[r0s0]"]
        )

        X_exog.loc[:, "spread_[VTB_weighted_rate_VTB_weighted_rate[r0s0]]"] = (
            X.loc[:, "VTB_weighted_rate_[priv]"]
            - X.loc[:, "VTB_weighted_rate_[priv]_[r0s0]"]
        )

        X_exog = generate_svo_flg(X_exog)

        X_exog.loc[:, self.SP_NAME] = (
            X.loc[:, "VTB_weighted_rate_[priv]"] - X.loc[:, "SBER_max_rate"]
        )

        X_exog = X_exog[self.full_features + [self.SP_NAME]]
        return X_exog

    def _generate_features_to_predict(self, X):
        if (
            X.index[0] - self.default_start_date
        ).days <= 31:  # Для случая прогноза in sample
            X = self._generate_features(X)
        elif (X.index[0] - self.X_stack.index[-1]).days > 40:
            raise ValueError(
                "More than 40 days between last train date (or last predict date) and current first predict date"
            )
        elif (X.index[0] - self.X_stack.index[-1]).days < 0:
            raise ValueError(
                "Less than 0 days between last train date (or last predict date) and current first predict date"
            )
        else:
            X_added = pd.concat([self.X_stack, X])
            self.X_stack = X_added.iloc[-1:, :]
            X_added = self._generate_features(X_added)
            X = X_added.iloc[1:, :]

        return X

    def fit(self, X: pd.DataFrame, y):
        verify_data(X, self.features, y, self.target)
        self.X_stack = X.iloc[-1:, :]
        X = self._generate_features(X)[self.full_features]

        if isinstance(y, pd.DataFrame):
            self.estimator.fit(X, np.ravel(y.values / 1e9))
        else:
            self.estimator.fit(X, y / 1e9)

    # добавляем корректирующие блоки перед предиктом

    def _sigmoid(self, x):
        return 1 / (1 + np.exp(-x))

    def _f_act_priv(self, x):
        if x < 1:
            return max(
                0.2,
                2
                * self._sigmoid((x / 2) ** 2 * np.sign(x) + (x / 4) ** 4 * np.sign(x)),
            )
        else:
            return (
                4 * self._sigmoid((x / 2) ** 2 * np.sign(x) + (x / 4) ** 4 * np.sign(x))
                - 1.126
            )

    def _correct_pred_feature(self, df, y):
        coef = df[self.SP_NAME].apply(lambda x: self._f_act_priv(x))

        return coef * y

    def predict(self, X: pd.DataFrame):
        verify_data(X, self.features)
        # заполняем пропуски в случае задания крайнего сценария
        X = self._generate_features_to_predict(X).fillna(0)
        y_hat = np.round(self.estimator.predict(X[self.full_features]) * 1e9, 2)
        y_hat = np.maximum(y_hat, self.min_model_predict)

        # корректируем
        y_hat = self._correct_pred_feature(X, y_hat).values

        y_hat = pd.DataFrame(data=y_hat, columns=self.target, index=X.index)
        y_hat.index.name = "report_dt"
        return y_hat


class NewbusinessPrivDataLoader(SimpleDataLoader):
    def __init__(self, config=CONFIG):
        super().__init__(config)

    def get_prediction_data(
        self,
        spark,
        start_date: datetime,
        end_date: datetime,
        params: Dict[str, Any] = None,
    ) -> Dict[str, pd.DataFrame]:
        pass


class NewbusinessPrivModelTrainer(SimpleModelTrainer):
    def __init__(self, config=CONFIG) -> None:
        for key, value in config.items():
            setattr(self, key, value)
        self.dataloader = NewbusinessPrivDataLoader()
        self.model = NewbusinessPrivModel()


class NewbusinessPrivModelAdapter(SimpleModelAdapter):
    def __init__(
        self, model_info_: ModelInfo, filepath_or_buffer, config: dict = CONFIG
    ):
        super().__init__(model_info_, filepath_or_buffer, config)

    def predict(
        self,
        forecast_context: ForecastContext,
        portfolio: pd.DataFrame = None,
        **params
    ):
        forecast_dates = forecast_context.forecast_dates
        df_date = forecast_context.model_data["features"].loc[forecast_dates, :]

        forecast_context.model_data["features"].loc[
            forecast_dates, "VTB_weighted_ftp_rate_[priv]"
        ] = calculate_weighted_ftp_rate(df_date, segment="priv")

        X = forecast_context.model_data["features"].loc[forecast_dates, self.features]
        pred = self._model_meta.predict(X)
        return pred


NewbusinessPriv = ModelMetaInfo(
    model_name=CONFIG["model_name"],
    model_trainer=NewbusinessPrivModelTrainer(),
    data_loader=NewbusinessPrivDataLoader(),
    adapter=NewbusinessPrivModelAdapter,
    segment="priv",
)
