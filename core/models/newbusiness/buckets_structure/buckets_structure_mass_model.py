...
CONFIG = {
    "features": [
        "VTB_rate_[mass]_[0_500k)",
        "VTB_rate_[mass]_[500k_1500k)",
        "VTB_rate_[mass]_[1500k_5000k)",
        "VTB_rate_[mass]_[5000k_15000k)",
        "VTB_rate_[mass]_[15000k_inf)",
    ],
    # тут важно пронать по порядку доли
    "balance_buckets": MASS_BALANCE_BUCKETS,
    # здесь задаем спреды
    "full_features": [
        "spread_VTB_rate_[mass]_[[500k_1500k)-[0_500k)]",
        "spread_VTB_rate_[mass]_[[1500k_5000k)-[500k_1500k)]",
        "spread_VTB_rate_[mass]_[[5000k_15000k)-[1500k_5000k)]",
        "spread_VTB_rate_[mass]_[[15000k_inf)-[5000k_15000k)]",
    ],
    "target": MASS_BALANCE_BUCKETS,
...
class NewbusinessBucketsMassModel:
    ...
    def _generate_features(self, X):
        ...
        X_exog.loc[:, "spread_VTB_rate_[mass]_[[500k_1500k)-[0_500k)]"] = (
            X.loc[:, "VTB_rate_[mass]_[500k_1500k)"]
            - X.loc[:, "VTB_rate_[mass]_[0_500k)"]
        )

        X_exog.loc[:, "spread_VTB_rate_[mass]_[[1500k_5000k)-[500k_1500k)]"] = (
            X.loc[:, "VTB_rate_[mass]_[1500k_5000k)"]
            - X.loc[:, "VTB_rate_[mass]_[500k_1500k)"]
        )

        X_exog.loc[:, "spread_VTB_rate_[mass]_[[5000k_15000k)-[1500k_5000k)]"] = (
            X.loc[:, "VTB_rate_[mass]_[5000k_15000k)"]
            - X.loc[:, "VTB_rate_[mass]_[1500k_5000k)"]
        )

        X_exog.loc[:, "spread_VTB_rate_[mass]_[[15000k_inf)-[5000k_15000k)]"] = (
            X.loc[:, "VTB_rate_[mass]_[15000k_inf)"]
            - X.loc[:, "VTB_rate_[mass]_[5000k_15000k)"]
        )
...
class NewbusinessBucketsMassDataLoader(SimpleDataLoader):
    def __init__(self, config=CONFIG):
...
class NewbusinessBucketsMassModelTrainer(SimpleModelTrainer):
    def __init__(self, config=CONFIG) -> None:
        for key, value in config.items():
            setattr(self, key, value)
        self.dataloader = NewbusinessBucketsMassDataLoader()
        self.model = NewbusinessBucketsMassModel()


class NewbusinessBucketsMassModelAdapter(SimpleModelAdapter):
    def __init__(
    ...
    def predict(
        ...
        **params,
        ...
        parse_res = parse_buckets_from_port(
            port, segment="mass", balance_buckets=self.balance_buckets
        )
        ...
        try:
            ...
            print(
                "Не заданы поля для ставок в разрезе балансов открытий вкладов для МАССОВОГО сегмента. Модель доли бакетов будет работать константано."
            )
        ...
        # генерируем признаки
        X_features = NewbusinessBucketsMassModel._generate_features(self, X=df_date)

        parse_res_new = calc_new_shares(
            parse_res, self.balance_buckets, "mass", X_features
        )

        pred = pd.DataFrame(data=parse_res_new, index=forecast_dates).add_prefix(
            "bucket_balance_share_[mass]_"
        )

        return pred


NewbusinessBucketsMass = ModelMetaInfo(
    model_name=CONFIG["model_name"],
    model_trainer=NewbusinessBucketsMassModelTrainer(),
    data_loader=NewbusinessBucketsMassDataLoader(),
    adapter=NewbusinessBucketsMassModelAdapter,
    segment="mass",
)
