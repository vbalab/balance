...
CONFIG = {
    "common_params": {
        ...
        "dbase_name": "prod_dadm_alm_sbx.almde_fl_dpst_early_close",
...
class MetaDataLoader(Meta):
    ...
    def get_training_data(
        ...
        weird_flag = MetaDataLoader._gen_weird_flag(data)
        data = data[~weird_flag]
        return {"data": data}
    ...
    def get_ground_truth(
        ...
        return {"target": data.groupby(_REPORT_DT_COLUMN).agg({"SER_d_cl": "sum"})}
...
class MetaAdapter(Meta):
    ...
    def create_newbusiness_generations(self, curr_portfolio, curr_features=None):
        first_pred_date = self.forecast_dates[0]
        weighted_rates = self._gen_external_weighted_rate(first_pred_date)
        ...
        for pred_date in [d for d in self.forecast_dates if d >= first_pred_date]:
            ...
            # приджойним поля с бакетами балансов из признаков
            bucket_cols = [
                s for s in list(curr_features.columns) if "bucket_balance_share_" in s
            ]

            #             print('curr_features', curr_features)
            #             print('f_port', f_port)

            f_port.loc[:, bucket_cols] = curr_features.loc[
                pred_date.strftime("%Y-%m-%d"), bucket_cols
            ].values[0]
        ...
        return pd.concat(res, ignore_index=True)[PORTFOLIO_COLUMNS_ + bucket_cols]
    ...
    def predict(
        ...
        **params,
        ...
        curr_features = forecast_context.model_data["features"]
        ...
        newbusiness_pred = self.create_newbusiness_generations(
            curr_portfolio, curr_features
        )
